...
The data I used for this case study came from the BirdCLEF 2024 Kaggle competition \citep{birdclef-2024}. This dataset consists of over 20,000 audio recordings of bird calls from the \url{xeno-canto.org} website. From each recording, I sampled a random 10 second chunk and transformed them into mel spectrograms using the R package "torchaudio" \citep{keydanaTorchaudioInterfacePytorchs2023}. Mel spectrograms are frequency-time spectrograms in which the frequency values are transformed to the Mel scale, which more accurately represents how humans perceive sound (humans can detect frequency differences better at lower frequencies). I took the hyperparameters used for this transformation (e.g. n\_fft, n\_mels) from the winning solution of the Kaggle competition. After the transformation, I normalized each spectrum to have values between 0 and 1. In addition to the audio recordings, the dataset also provides the longitude and latitude values of the location where they were recorded. After discarding audio recordings with bad ratings or missing longitude/latitude values, I ended up with 21166 samples for 177 different bird species.

I used this data to train the three model architectures that \pkg{cito} now supports: Fully connected neural networks were trained on the longitude and latitude values, convolutional neural networks were trained on the mel spectrograms, and multimodal neural networks were trained on both the longitude/latitude values and the mel spectrograms. Since the 177 classes are highly unbalanced (class sizes range from 5 to 466), I divided my data into 5 folds, preserving the class distributions in each fold, which are used in a cross-validation to evaluate the performance of the model architectures. In total, I trained 15 models (5 DNNs, 5 CNNs and 5 MMNs).

In this case study, I pretended to be a user with little to no prior knowledge of deep learning and network architectures. Therefore, I mostly used the default values for the hyperparameters:
...
In this work, I introduced the implementations of CNN and MMN architectures in the R package \pkg{cito}. Building and training CNNs consisting of convolutional, pooling and linear layers previously required users to have extensive knowledge of complex deep learning frameworks such as PyTorch or TensorFlow. The simple interface I provide, makes this not necessary anymore. Because of the \pkg{torch} backend and its provided GPU support, \pkg{cito} can train these large networks as efficiently as the complex frameworks. Moreover, I provide the option of utilizing transfer learning by integrating several pre-trained models. To address the current trend of multimodal models, I implemented an MMN architecture that can combine the two neural network architectures currently available in \pkg{cito} (DNN and CNN). With the example of bird species classification, I showcased how little code is necessary when using \pkg{cito} to train good performing, CNN-based classifiers.

The CNN architecture performed best in my case study (macro-averaged AUC: 0.967, accuracy: 0.673). However, these results can not be compared to the results of the Kaggle competition, because I evaluated my models on the training set via cross-validation, whereas the models of the Kaggle competition have been evaluated on a separate test set, which has significant differences to the training set: 1) Unlike the training set, which contains sound recordings from all over the globe, the sound recordings of the test set only contains sound recordings from the western ghats of india. This local blocking of the test set might reduce the models performance, since the model trained on the global data might have overfitted to global patterns such as background noises or to local dialects of the bird calls, that are not present in the local region of the test set. 2) The sound recordings of the test set have a fixed length of 4 minutes, whereas the median length of sound recordings in the training set is only 22.7 seconds. A large portion of the training recordings are shorter or only a bit longer than 10 seconds, so you can expect that the randomly sampled 10 seconds chunk contains at least parts of the bird call, that can be used for prediction. The probability that a random 10 seconds chunk of the 4 minute long audio recordings of the test set contains parts of the bird call, instead of only noise, is lower. A way to address this problem is to divide the audio recordings into 10 seconds chunks and then average the predictions of these chunks. Since there might be chunks with no or only little parts of the bird call and mostly background noise, this can increase the uncertainty of the final predictions, which reduces the overall performance measured by AUC. 3) The test set only includes the audio files, not the longitude and latitude values. This makes it impossible to evaluate the DNNs and MMNs I trained on the test set, since they need them to make predictions.
