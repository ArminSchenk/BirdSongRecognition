...
\section*{DNNs in \pkg{cito}}
\addcontentsline{toc}{chapter}{DNNs in \pkg{cito}}
The current version of \pkg{cito} only supports deep neural networks (DNNs), also known as fully connected neural networks or multilayer perceptrons, which can be build and trained using the \fn{dnn} function. The architecture of the DNNs can be specified in the \fn{dnn} function using three arguments. The first argument is a numeric vector. The length of this vector determines the number of hidden layers and the values determine the number of neurons in these layers. The other two arguments (bias and dropout) determine whether the neurons have a learnable bias and what dropout rate is applied to the neurons during training.
